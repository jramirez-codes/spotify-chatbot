...
function App() {
  let spotifyToken: SpotifyToken = createClientToken()
  let [spotifyUserData, setSpotifyUserData] = React.useState<any>(null)

  // Fetching User Info
  React.useEffect(() => {
    let key = 'spotify-data'
    let userFavorite = window.localStorage.getItem(key)
    if (!userFavorite) {
      const asnycFunc = async () => {
        let [topArtist, topSongs] = await fetchUserFavorite(spotifyToken)
          ...
          <div className='fixed top-0 left-0'>
            <div className="flex shink-0 animate-newScroll">
              {spotifyUserData.topArtist.items.map((obj: any, idx: any) => {
                let size = Math.floor(Math.random() * (35 - 10 + 1)) + 10;
                let margin = Math.floor(Math.random() * (20 - 3 + 1)) + 3;
                  ...
                  return (
                    <div style={{ margin: margin }} className="block">
                      <img
                        ...
                        src={obj.images[1].url} key={idx + obj.images[0].url}
                      />
                    </div>
                  )
                }
